        ...
        <OwlCarousel options={owlOptions}>
            {_.map(slides, (slide) => (
                <div className="header-content d-flex" key={slide.order} style={{
                    backgroundImage: `url(${slide.bg_image !== null ? (slide.bg_image) : default_bg})`,
                        ...
                        <div className="col-md-6">
                            <div className="h-100 w-100 text-uppercase fl-sm">
                                <div className="w-100 m-l">
                                    <div className="mb-4">
                                        <h3 className={"lead my-leading"}>{slide.leading}</h3>
                                        {slide.article_related.reduction > 0 && (
                                            <small className="badge small-text badge-danger"
                                                   style={{
                                                       fontWeight: "400"
                                                   }}>
                                                {slide.article_related.reduction} % de réduction
                                            </small>
                                        )}
                                        <h2 className={"my-title"}>{slide.title} </h2>

                                        <h4 className={"my-subtitle"}>{slide.subtitle} </h4>
                                    </div>

                                    <Link to={`/p/${slide.article_related.slug}`}
                                          className="btn btn-danger">Découvrez les détails</Link>
                                </div>
                            </div>
                        </div>
